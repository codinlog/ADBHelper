...
static inline  unsigned long adb_thread_id()
{
...
static inline int unix_read(int fd, void* buf, size_t len) {
...
// with _setmode().
static inline int  unix_open(const char* path, int options, ...)
{
...
static inline int  adb_shutdown(int fd)
{
	return shutdown(fd, SHUT_RDWR);
}
static inline int  adb_shutdown(int fd, int direction)
{
...
static inline int   adb_lseek(int  fd, int  pos, int  where)
{
