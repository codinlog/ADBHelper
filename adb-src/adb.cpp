...
std::string adb_version() {
	return "maked by codinlog@foxmail.com";
...
std::string get_connection_string() {
	...
	connection_properties.push_back(StringPrintf(
		"features=%s", FeatureSetToString(supported_features()).c_str()));
	return StringPrintf(
		"%s::%s", adb_device_banner,
		Join(connection_properties, ';').c_str());
}
...
void send_connect(atransport* t) {
	...
	if (connection_str.length() > MAX_PAYLOAD_V1) {
		std::cout << "Connection banner is too long (length = "
...
static void ReportServerStartupFailure(DWORD pid) {
	...
	//std::string header = StringPrintf("--- adb starting (pid %d) ---", pid);
...
int launch_server(const std::string& socket_spec) {
	...
	const DWORD module_result = GetModuleFileNameW(NULL, program_path,
		std::wcslen(program_path));
	if ((module_result >= std::wcslen(program_path)) || (module_result == 0)) {
	...
	WCHAR   args[64];
	std::swprintf(args, std::wcslen(args), L"adb -L %s fork-server server --reply-fd %d",
	...
	// Wait for the "OK\n" message, for the pipe to be closed, or other error.
	{
		...
		if (ReadFile(ack_read.get(), temp, sizeof(temp), &count, NULL)) {
			...
			const DWORD expected_length = std::strlen(expected) - 1;
...
int handle_forward_request(const char* service, TransportType type, const char* serial, int reply_fd)
{
	...
	if (!strncmp(service, "forward:", 8) || !strncmp(service, "killforward:", 12)) {
		...
		std::vector<std::string> pieces = Split(service, ";");

		if (kill_forward) {
			// Check killforward: parameter format: '<local>'
			if (pieces.size() != 1 || pieces[0].empty()) {
				SendFail(reply_fd, StringPrintf("bad killforward: %s", service));
				return 1;
			}
		}
		else {
			...
			if (pieces.size() != 2 || pieces[0].empty() || pieces[1].empty() || pieces[1][0] == '*') {
				SendFail(reply_fd, StringPrintf("bad forward: %s", service));
				return 1;
			}
		}
		...
		case INSTALL_STATUS_CANNOT_BIND:
			message = StringPrintf("cannot bind listener: %s",
				error.c_str());
			break;
		case INSTALL_STATUS_CANNOT_REBIND:
			message = StringPrintf("cannot rebind existing socket");
			break;
		case INSTALL_STATUS_LISTENER_NOT_FOUND:
			message = StringPrintf("listener '%s' not found", service);
			break;
		}
...
int handle_host_request(const char* service, TransportType type,
	...
	if (!strncmp(service, "disconnect:", 11)) {
		...
		if (!android::base::ParseNetAddress(address, &host, &port, &serial, &error)) {
			return SendFail(reply_fd, StringPrintf("couldn't parse '%s': %s",
				address.c_str(), error.c_str()));
		}
		atransport* t = find_transport(serial.c_str());
		if (t == nullptr) {
			return SendFail(reply_fd, StringPrintf("no such device '%s'",
				serial.c_str()));
		}
		kick_transport(t);
		return SendOkay(reply_fd, StringPrintf("disconnected %s", address.c_str()));
	}

	// Returns our value for ADB_SERVER_VERSION.
	if (!strcmp(service, "version")) {
		return SendOkay(reply_fd, StringPrintf("%04x", ADB_SERVER_VERSION));
	}
